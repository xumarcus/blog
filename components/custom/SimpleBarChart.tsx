...
// #3182bd to match line chart
const SimpleBarChart = ({ data, formatter, name, title }: Props) => (
  <>
    {title && (
    ...
    <ResponsiveContainer aspect={2.0} height="100%" width="100%">
      <BarChart data={data.map(([x, value]) => ({ x, value }))}>
        ...
        <Legend />
        <Bar name={name} fill="#3182bd" dataKey="value" />
      </BarChart>
    </ResponsiveContainer>
  </>
)
